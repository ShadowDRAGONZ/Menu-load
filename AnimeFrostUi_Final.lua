...
local function applyBackground(window)
    ...
    local panelHost = mainBar and mainBar:FindFirstChild("Background")
    local host = (typeof(panelHost) == "Instance" and panelHost) or root
    ...
    layer.Active = false
    layer.ZIndex = math.max(0, (tonumber(host.ZIndex) or 1) - 1)
    layer.Parent = host
    trackInstance(layer)
    ...
    task.spawn(function()
        local asset, loadedFromURL = getOnlineImage(backgroundConfig.URL, backgroundConfig.FilePath)
        if image and image.Parent and asset then
            image.Image = asset
        end
    ...
    if backgroundConfig.ParallaxEnabled then
        local okTween, parallaxTween = pcall(function()
            return TweenService:Create(
                image,
                TweenInfo.new(backgroundConfig.ParallaxDuration, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
                {
        ...
        if okTween and parallaxTween then
            trackTween(parallaxTween)
            pcall(function() parallaxTween:Play() end)
        end
    end
    ...
    colorGradient.Transparency = NumberSequence.new({
        ...
        ColorSequenceKeypoint.new(0.35, 0.42),
    })
    -- fix typo in original: use NumberSequenceKeypoint for transparency
    colorGradient.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0.00, 0.15),
...
local function createSnowParticles(layer)
    ...
    local symbols = {"•", "·", "✦", "❄", "✧"}
    local interval = 1 / math.max(tonumber(cfg.ParticlesPerSecond) or 1, 0.1)
    ...
    local function animateParticle(particle, initial)
        task.spawn(function()
            ...
            while typeof(particle) == "Instance" and particle.Parent and holder.Parent do
                local ok = pcall(function()
                    local startX = random:NextNumber(-0.05, 1.05)
                    local drift = random:NextNumber(-0.16, 0.16)
                    local size = random:NextInteger(tonumber(cfg.MinSize) or 8, tonumber(cfg.MaxSize) or 21)
                    local duration = random:NextNumber(tonumber(cfg.MinDuration) or 8, tonumber(cfg.MaxDuration) or 17)
                    particle.TextSize = size
                    particle.Text = symbols[random:NextInteger(1, #symbols)]
                    particle.Position = UDim2.fromScale(startX, random:NextNumber(-0.18, -0.03))
                    particle.Rotation = random:NextNumber(-25, 25)
                    particle.TextTransparency = random:NextNumber(cfg.MinTransparency or 0.2, cfg.MaxTransparency or 0.68)

                    local fallTween = tween(
                        particle,
                        duration,
                        {
                            Position = UDim2.fromScale(startX + drift, 1.10),
                            Rotation = particle.Rotation + random:NextNumber(-160, 160),
                    ...
                    if fallTween then
                        pcall(function() fallTween.Completed:Wait() end)
                    end
                end)
                if not ok then
                    break
                end
                task.wait(random:NextNumber(cfg.RespawnDelayMin or 0.05, cfg.RespawnDelayMax or 0.55))
            end
        end)
    end

    local count = math.max(1, math.floor(tonumber(cfg.Count) or 20))
...
local function playLoading(window)
    ...
    local root = window.UIElements.Main
    local container = window.UIElements.MainContainer
    ...
    local ok, result = pcall(function()
        ...
        if typeof(container) == "Instance" then
            local scale = container:FindFirstChild("AF_OpenScale")
            if not scale then
                scale = Instance.new("UIScale")
            ...
            tween(scale, CONFIG.Loading.FadeDuration + 0.10, {Scale = 1}, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        end
...
local function animateWindowOpen(window)
    ...
    pcall(function()
        local scale = container:FindFirstChild("AF_FinalOpenScale")
        if not scale then
            scale = Instance.new("UIScale")
            scale.Name = "AF_FinalOpenScale"
            scale.Scale = 0.94
            scale.Parent = container
            trackInstance(scale)
        else
