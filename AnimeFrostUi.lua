...
local function startGradientRunner()
    ...
    gradientRunner = RunService.Heartbeat:Connect(function()
...
local function loadWebImage(url, filePath, fallback)
    ...
    if not hasFile then
        ...
        local success = pcall(writefile, filePath, body)
        if not success then
            return fallback, false
        end
    end

    local success, asset = pcall(customAsset, filePath)
...
local function applyBackground(window)
    ...
    task.spawn(function()
        local asset, loadedFromURL = loadWebImage(backgroundConfig.URL, backgroundConfig.FilePath, backgroundConfig.FallbackAssetId)
    ...
    if backgroundConfig.ParallaxEnabled then
        local parallaxTween = TweenService:Create(
            image,
...
local function createSnowParticles(layer)
    ...
    if not particleConfig.Enabled or not layer then
        return
    end
